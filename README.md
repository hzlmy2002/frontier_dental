...
> **JSON-LD** (JSON for Linking Data) is a standard way for websites to
> embed structured product metadata — name, SKU, price, images,
> breadcrumbs — directly in the page HTML inside a `<script type="application/ld+json">`
> tag. When present, it's the cheapest and most reliable extraction source.
...
3. **Product detail pages ship rich JSON-LD.** `Product` and `BreadcrumbList` give SKU,
...
| Product detail page fields | `GET /product/<slug>` → JSON-LD `Product` + `BreadcrumbList` + `window.masterData` are all server-rendered | none |
...
HTTP vs. ~14 s of browser. Product detail page extraction has the same shape — httpx +
...
2. **Tier-3 LLM** — for irregular product detail pages. When a page is missing JSON-LD
...
never fired on this site — every product detail page we've inspected ships its
structured data server-side.

If you're porting this design to a site **without** intent-driven
discovery and **with** uniform server-rendered product detail page templates, you can
...
| **PlaywrightFetcher** | [src/frontier_dental/playwright_fetcher.py](src/frontier_dental/playwright_fetcher.py) | Lazy headless Chromium for Tier 2 product detail page re-rendering. Separate browser from Navigator's. |
...
| **Storage** | [src/frontier_dental/storage.py](src/frontier_dental/storage.py) | SQLite `products` (SKU primary key) + `crawl_state`. CSV / JSON export. |
...
Most Safco product detail pages are **grouped products** — one parent SKU with multiple
...
order). Single-variant product detail pages yield one-element tuples, so consumers can
treat both shapes uniformly.

| Field | Type | Source |
|---|---|---|
| `sku` | string (primary key) | JSON-LD `Product.sku` |
...
- `specifications` is typically empty for this site — the per-product-page
